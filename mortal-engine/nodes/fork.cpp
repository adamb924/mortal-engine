...
QSet<MorphemeSequence> Fork::possibleMorphemeSequences(const QSet<MorphemeSequence> given, QHash<const Jump *, int> &jumps) const
{
    qDebug() << "Fork::possibleMorphemeSequences" << debugIdentifier();// << "Given:" << given;
    ...
    if( optional() )
    {
        ...
        result.unite(given);
//        qDebug() << "Added given:" << result;
    }
//    else
//    {
//        qDebug() << "Fork not optional!";
//    }
    if( given.isEmpty() )
    {
        ...
        foreach(Path * path, mPaths)
        {
            result.unite( path->possibleMorphemeSequences(result, jumps) );
        }
        if( optional() )
        {
    ...
    else
    {
        ...
        while(iter.hasNext())
        {
            ...
            foreach(Path * path, mPaths)
            {
                result.unite( path->possibleMorphemeSequences(given, jumps) );
            }
        }
        */
        foreach(Path * path, mPaths)
